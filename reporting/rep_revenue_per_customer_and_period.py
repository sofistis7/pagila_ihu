...
, customers as (
 select *
 from studied-brand-481913-u8.staging_db.stg_customer
)
...
, revenues_per_period as (
 ...
 from revenues
 left join customers on payment_customer_id = customers.customer_id
 group by 1,2,3
union all
 select
...
select
  ...
  , revenues_per_period.customer_id
  , revenues_per_period.total_revenues
 from reporting_dates
inner join revenues_per_period
